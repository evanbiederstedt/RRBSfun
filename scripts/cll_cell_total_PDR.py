...
cll_cells2 = glob.glob("cll*")
cll_cells3 = glob.glob("RRBS_NormalB_CLL*")

import itertools
cll = itertools.chain(cll_cells1, cll_cells2, cll_cells3)
cll_cells = list(cll)

newdf1 = pd.DataFrame()
for filename in cll_cells:
    df = pd.read_table(filename)
    df['filename'] = str(filename)
    df = df.drop(['start', 'strand', 'thisMeth', 'thisUnmeth', 'avgWeightedEnt', 'CpGEntropy', 'avgReadCpGs',
    ...
    df = df.sum()
    df['PDR'] = df['mixedReadCount']/df['total_reads']
    newdf1 = newdf1.append(df, ignore_index=True)

newdf1 = newdf1.drop(['chr'], axis=1)

newdf1 = newdf1[['filename', 'PDR', 'methReadCount', 'unmethReadCount', 'mixedReadCount', 'total_reads']]

newdf1.to_csv("CLL_cells_PDR_total_chr2_5_11.csv")
